...
pub fn find_unconstrained_division(cfg: &Cfg) -> ReportCollection {
    ...
    for basic_block in cfg.iter() {
        for stmt in basic_block.iter() {
            update_divisors(stmt, &mut divisors); // 收集所有`c <-- a / b`Statement中的除数b
        }
    }
    ...
    for basic_block in cfg.iter() {
        for stmt in basic_block.iter() {
            update_constraints(stmt, &mut constraints); // 收集所有`IsZero`（circomlib）模板实例化语句
        }
    }
...
fn update_constraints(stmt: &Statement, constraints: &mut HashMap<VariableAccess, Component>) {
    ...
    match stmt {
        ...
        // 即识别形如`var[i] = T(...)`的语句，并判断T是否为IsZero
        Substitution { meta, var, op: AssignLocalOrComponent, rhe } => {
            ...
            // 检查rhe是否为Update{access, rhe, ..}这种模式，
