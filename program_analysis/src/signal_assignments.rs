...
/// 用于表示不必要的信号赋值警告，当右侧表达式已经是二次项时，应该使用<==而不是<--
...
struct Constraint {
    pub meta: Meta,
    pub lhe: Expression, // 左表达式
    pub rhe: Expression, // 右表达式
}
...
/// 遍历【控制流图cfg】中的所有语句，查找信号赋值操作，生成相应报告
pub fn find_signal_assignments(cfg: &Cfg) -> ReportCollection {
    ...
    // 遍历语句，识别信号赋值和约束，并更新SignalUse结构体
    // 赋值都被封装为Assignment结构体，加入的SignalUse的assignments这一set
    // 约束都被封装为Constraint结构体，加入的SignalUse的constraints这一set
