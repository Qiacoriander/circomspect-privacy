...
pub fn find_unconstrained_less_than(cfg: &Cfg) -> ReportCollection {
    ...
    for basic_block in cfg.iter() {
        for stmt in basic_block.iter() {
            update_components(stmt, &mut components); // 收集所有`LessThan`和`Num2Bits`模板实例var` 到`components`
        }
    }
    ...
    // 遍历收集到的两个函数的输入，分别处理
    // 对于 LessThan 组件的输入：记录表达式值及其元数据
    // 对于 Num2Bits 组件的输入：记录表达式值、元数据和位大小
    // 记录一个表达式既是LessThan输入又是Num2Bits输入的机制 ：
    //  1.同一个表达式（比如变量 x ）可能在代码中多次出现
    //  2.当它作为LessThan的输入时（如 lt.in[0] <== x ），会被记录在 constraints[x].less_than 中
    //  3.当它作为Num2Bits的输入时（如 n2b.in <== x ），会被记录在 constraints[x].num_2_bits 和 constraints[x].bit_sizes 中
    //  4.由于使用了同一个表达式作为键，所以可以在后续分析中知道这个表达式既是LessThan的输入又是Num2Bits的输入
...
fn update_components(stmt: &Statement, components: &mut HashMap<VariableAccess, Component>) {
    ...
    if let Substitution { meta, var, op: AssignLocalOrComponent, rhe, .. } = stmt {
        ...
        // Substitution可能发生在本地变量local、信号signal、组件component上
        // 这里只关注组件component的情况
        ...
        // Statement是Substitution类型，且RHS是Update类型，说明是对组件component的赋值
        let (rhe, access) = if let Update { access, rhe, .. } = rhe {
            // 这里rhe重新赋值了，新的rhe变量是Update结构体内部的rhe字段引用
            (rhe.as_ref(), access.clone())
        } else {
            (rhe, Vec::new())
        };
        // 是将组件赋值实例化的语句，match一下看是否是LessThan和Num2Bits模板实例化，是则记录到components中
        // 判断rhe是否是组件实例化调用（Call），这个rhe实际是Update这个Expression的rhe
...
fn update_inputs(
    ...
    use Expression::*;
    use AccessType::*;
    // match一下约束赋值语句（只关注形如component.signal <== expression的语句）
    if let Substitution {
    ...
    } = stmt
    {
        ...
        if let Some(Component::Num2Bits { bit_size, .. }) = components.get(&component) {
            ...
            // 如果找到了关联的Num2Bits组件，继续检查信号访问是否为"in"
            if signal_name != "in" {
                return;
            }
            trace!("`Num2Bits` input signal assignment `{rhe}` found");
            // 将找到的输入信号信息添加到inputs向量中
