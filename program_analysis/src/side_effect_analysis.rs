...
pub fn run_side_effect_analysis(cfg: &Cfg) -> ReportCollection {
    ...
    let signal_decls = cfg
        ...
        .filter_map(|(name, declaration)| {
            if matches!(declaration.variable_type(), VariableType::Signal(_, _, _)) {
    ...
    let exported_signals = signal_decls
        ...
        .filter_map(|(name, declaration)| {
            if matches!(
                ...
                VariableType::Signal(SignalType::Input | SignalType::Output, _, _)
            ) {
                Some(*name)
            } else {
                None
    ...
    use Statement::*;
    // 添加在声明Declaration、返回值Return、断言Assert、条件语句IfThenElse中出现的变量
    for basic_block in cfg.iter() {
    ...
    // 对于每个变量定义，分析器检查：
    // 1.如果变量从未被读取 → 生成"未使用变量"警告
    // 2.如果变量不污染任何汇点 → 生成"无副作用变量"警告
    ...
    // large (65536 element) arrays.
    // 对于每个信号定义，分析器检查：
    // 1.如果信号从未被读取 → 生成"未使用信号"警告
    // 2.如果信号不污染任何约束变量 → 生成"无约束信号"警告
