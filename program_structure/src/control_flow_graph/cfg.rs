...
impl Cfg {
    ...
    pub fn output_signals(&self) -> impl Iterator<Item = &VariableName> {
        ...
        self.declarations.iter().filter_map(|(name, declaration)| {
            match declaration.variable_type() {
                Signal(Output, _, _) => Some(name),
                _ => None,
            }
        })
    }

    /// Returns an iterator over private input signals of the CFG.
    pub fn private_input_signals(&self) -> impl Iterator<Item = &VariableName> {
        use SignalType::*;
        use VariableType::*;
        self.declarations.iter().filter_map(|(name, declaration)| {
            match declaration.variable_type() {
                Signal(Input, _, true) => Some(name),
                _ => None,
            }
        })
    }

    /// Returns an iterator over public input signals of the CFG.
    pub fn public_input_signals(&self) -> impl Iterator<Item = &VariableName> {
        use SignalType::*;
        use VariableType::*;
        self.declarations.iter().filter_map(|(name, declaration)| {
            match declaration.variable_type() {
                Signal(Input, _, false) => Some(name),
                _ => None,
            }
        })
    }
