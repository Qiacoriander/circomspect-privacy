    ...
    fn try_lift(&self, _: (), reports: &mut ReportCollection) -> IRResult<Self::IR> {
        match self {
            ...
            ast::Expression::Call { meta, id, args } => Ok(ir::Expression::Call {
                ...
                // Initially set to None, will be populated later during CFG linking phase
                target_cfg: None,
            }),
...
impl TryLift<()> for ast::VariableType {
    ...
    fn try_lift(&self, _: (), reports: &mut ReportCollection) -> IRResult<Self::IR> {
        match self {
            ...
            ast::VariableType::Signal(signal_type, tag_list, is_private) => {
                Ok(ir::VariableType::Signal(
                    signal_type.try_lift((), reports)?,
                    tag_list.clone(),
                    *is_private,
                ))
            }
