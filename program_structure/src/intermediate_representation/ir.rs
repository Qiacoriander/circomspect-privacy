...
// Forward declaration for CFG reference
use crate::cfg::Cfg;
pub type WeakCfgRef = Weak<RefCell<Cfg>>;
...
pub enum VariableType {
    ...
    Component,
    AnonymousComponent,
    Signal(SignalType, TagList, bool), // bool 表示是否 `private`
}
...
impl fmt::Display for VariableType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        ...
        match self {
            ...
            Signal(signal_type, tag_list, is_private) => {
                write!(f, "signal")?;
                if *is_private {
                    write!(f, " private")?;
                }
