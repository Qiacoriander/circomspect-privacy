...
template TotalColorMatches(n, nColors) {
    ...
    for (var c = 0; c < nColors; c++) {
        ...
        min_count[c] = Min(4); // max count per color is 4 (fits in 4 bits)
...
/// Computes exact and partial matches, checks they match the claimed hint.
template VerifyHint(nPegs, nColors) {
    ...
    // (claimed_exact - exact.count) * active === 0
...
/// Verifies that all hints are correct for a committed secret code.
///
...
template Mastermind(nPegs, nColors, maxTurns) {
    ...
    // Track if any turn had exact_matches == nPegs (code was solved)
    ...
    for (var t = 0; t < maxTurns; t++) {
        ...
        turn_active[t] = LessThan(8); // 8 bits enough for maxTurns
        ...
        component is_solved = IsEqual();
        is_solved.in[0] <== hints[t * 2]; // exact matches
        is_solved.in[1] <== nPegs;
        // Only count as solved if turn is active
        solved_flags[t] <== is_solved.out * turn_active[t].out;
        solved_accumulator[t + 1] <== solved_accumulator[t] + solved_flags[t];
    }

    // ---- Verify winner ----
    // was_solved = 1 if any active turn had exact_matches == nPegs
    signal was_solved;
    ...
    // If solved: winner must be 2 (codebreaker)
    // If not solved: winner must be 1 (codemaker)
    // expected_winner = was_solved * 2 + (1 - was_solved) * 1
    //                = was_solved * 2 + 1 - was_solved
    //                = was_solved + 1
    signal expected_winner <== was_solved + 1;
    expected_winner === winner;
}

// Instantiate: 4 pegs, 6 colors, 8 max turns
component main {public [code_hash, num_turns, guesses, hints, winner]} = Mastermind(4, 6, 8);
