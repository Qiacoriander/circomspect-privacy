...
// for each proof, update 2**m transactions
    ...
    // Declare components in the initial scope
    component lessThan[2**m];
    component greaterThan[2**m];
    component isEqual[2**m];
    ...
    for (var i = 0; i < 2**m; i++) {
        log("==========================================");
        log("Transaction", i, "Verification Details:");
        log("==========================================");
        
        // Log transaction data hash inputs
        log("Input Data:");
        log("- FromX:", fromX[i]);
        log("- FromY:", fromY[i]);
        log("- FromIndex:", fromIndex[i]);
        log("- ToX:", toX[i]);
        log("- ToY:", toY[i]);
        log("- NonceFrom:", nonceFrom[i]);
        log("- Amount:", amount[i]);
        log("- TokenType:", tokenTypeFrom[i]);
        log("- R8x:", R8x[i]);
        log("- R8y:", R8y[i]);
        log("- S:", S[i]);
        
        log("Merkle Tree Info:");
        log("- TxRoot:", txRoot);
        ...
        log("Merkle Proof Details:");
        for (var j = 0; j < m; j++) {
            log("Level", j, ":");
            log("  - Path Element:", paths2txRoot[i][j]);
            log("  - Position (0=left, 1=right):", paths2txRootPos[i][j]);
        ...
        log("Balance Tree Info:");
        log("- Current State:", currentState);
        log("- Intermediate Root[2*i]:", intermediateRoots[2*i]);
        log("- Intermediate Root[2*i + 1]:", intermediateRoots[2*i + 1]);
        log("- Intermediate Root[2*i + 2]:", intermediateRoots[2*i + 2]);
        ...
        greaterThan[i] = GreaterThan(32);
        isEqual[i] = IsEqual();
        // Thay thế balanceFrom[i] - amount[i] <= balanceFrom[i]
        greaterThan[i].in[0] <== balanceFrom[i];
        greaterThan[i].in[1] <== amount[i];
        greaterThan[i].out === 1; // Ensure balanceFrom[i] >= amount[i]
        // Thay thế balanceTo[i] + amount[i] >= balanceTo[i]
        lessThan[i].in[0] <== balanceTo[i];
        lessThan[i].in[1] <== balanceTo[i] + amount[i];
        lessThan[i].out === 1; // Ensure balanceTo[i] + amount[i] >= balanceTo[i]

        // Thay thế nonceFrom[i] != NONCE_MAX_VALUE
        isEqual[i].in[0] <== nonceFrom[i];
        isEqual[i].in[1] <== NONCE_MAX_VALUE;
        isEqual[i].out === 0; // Ensure nonceFrom[i] != NONCE_MAX_VALUE
