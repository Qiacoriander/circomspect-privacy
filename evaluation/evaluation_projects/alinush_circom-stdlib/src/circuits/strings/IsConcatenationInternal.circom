...
template IsConcatenationInternal(N, N_1, N_2) {
    ...
    // Get \tau^{|s_1|} from the array.
    // TODO(Perf): This adds unnecessary constraints past N - N_1
    //   Wouldn't it be more efficient to somehow do ArrayGet(N_1 + 1)(ptau[..(N-1)+1], s_1.len)?
    //   e.g., via a new ArrayGet(MAX_LEN, MAX_INDEX)
    signal tauToL1 <== ArrayGet(N)(ptau, s_1.len);

    // Recall that the concatenation check is:
    //   s(X)    =?= s_1(X)    + X^{|s_1|}    s_2(X)
    // This just checks it at X = \tau:
    //   s(\tau) =?= s_1(\tau) + \tau^{|s_1|} s_2(\tau)
