...
include "./lib/account_leaf/account_leaf.circom";
...
template StateUpdate(n, m) {
    ...
    // COMPUTING THE SENDER AND RECEIVER LEAVES -- BASE CASE
    component senderLeaves[2**m][2];
    component receiverLeaves[2**m][2];

    for (var i = 0; i < 2**m; i++) {
        // Initial state
        senderLeaves[i][0] = AccountLeaf();
        ...
        // Initial state
        receiverLeaves[i][0] = AccountLeaf();
        ...
        receiverLeaves[i][1] = AccountLeaf();
